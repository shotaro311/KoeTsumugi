...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState, warning: bool) -> &'static str {
    if warning && state == TrayIconState::Idle {
        return match theme {
            ...
            // but keep the independently branded warning fallback complete.
            AppTheme::Colored => "resources/app_idle_warning.png",
        };
    }
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/app_idle.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("KoeTsumugi v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("KoeTsumugi v{}", env!("CARGO_PKG_VERSION"))
    }
}
